...
<div class="bg-gradient-to-r from-primary/10 to-primary/5 rounded-xl p-5 mb-4 shadow-md">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex gap-2 items-center">
            ...
            <div class="dropdown dropdown-end">
                <label tabindex="0" class="btn btn-ghost btn-sm bg-base-100 hover:bg-base-200 border border-base-300 shadow-sm" aria-label="Filter">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" 
                    ...
                    <span class="badge badge-primary badge-xs ml-1" 
                          x-data="{ count: 0 }"
                          x-init="count = 
                            ({{ !empty($filters['types']) ? count($filters['types']) : 0 }}) + 
                            ({{ !empty($filters['account_id']) ? 1 : 0 }}) + 
                            ({{ !empty($filters['date_from']) ? 1 : 0 }}) +
                            ({{ !empty($filters['date_to']) ? 1 : 0 }})">
                        <span x-text="count"></span>
                    </span>
                </label>
                <div tabindex="0" class="dropdown-content z-[1] menu p-4 shadow-lg bg-base-100 rounded-xl w-80 border border-base-200">
                    ...
                    <div class="form-control mb-3">
                        ...
                        <div class="flex flex-row gap-1">
                            <label class="flex-1 cursor-pointer">
                                <input type="checkbox" class="hidden" wire:model.live="filters.types" value="1">
                                <div class="btn btn-sm btn-outline btn-primary w-full"
                                     :class="{ 'btn-active': filters.types.includes('1') }">
                                    <span>Income</span>
                                </div>
                            </label>
                            <label class="flex-1 cursor-pointer">
                                <input type="checkbox" class="hidden" wire:model.live="filters.types" value="2">
                                <div class="btn btn-sm btn-outline btn-secondary w-full"
                                     :class="{ 'btn-active': filters.types.includes('2') }">
                                    <span>Expense</span>
                                </div>
                            </label>
                        </div>
                    ...
                    <div class="form-control mb-3">
                        ...
                        <select class="select select-bordered select-sm w-full bg-base-100" wire:model.live="filters.account_id">
