...
new #[Layout('layouts.app')] class extends Component {
    ...
    {
        $percentage = ($this->category->transactions->sum('amount') / $this->category->budgets->limit_amount) * 100;
        return $percentage;
    }
};
?>
            ...
            <div class="w-1/2">
                <div class="dropdown dropdown-end w-full">
                    <label tabindex="0" class="btn btn-md border shadow-sm w-full"
                        ...
                        @if ($category && $category->budgets)
                            <span
                                class="badge badge-sm block truncate
                                @if ($this->getPercentage() < 50) badge-success @elseif($this->getPercentage() >= 50 && $this->getPercentage() < 100) badge-warning @else badge-error @endif">₱{{ number_format($category->budgets->limit_amount - $category->transactions->sum('amount')) }}</span>
                        @endif
                    </label>
                </div>
            </div>
