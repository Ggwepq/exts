    ...
    >
        <div class="relative">
            ...
            <div class="absolute top-2 right-2 flex space-x-2 z-20">
                <button 
                    class="btn btn-sm btn-circle btn-ghost text-white bg-base-300 bg-opacity-50 hover:bg-opacity-70" 
                    x-on:click="zoom(0.5)"
                    aria-label="Zoom in">
            ...
            <!-- Image Container -->
            <div 
                class="flex items-center justify-center h-[80vh] w-full overflow-hidden cursor-move"
            ...
            >
                <img 
                    :src="imageUrl" 
                    class="max-h-full object-contain will-change-transform"
                    :style="`transform: translate(${posX}px, ${posY}px) scale(${scale})`"
                    x-on:dblclick="scale === minScale ? zoom(1) : resetView()"
                    alt="Enlarged view"
                />
            </div>
            ...
            @if($withInstructions)
            <div class="absolute bottom-2 left-1/2 transform -translate-x-1/2 text-white text-xs bg-base-300 bg-opacity-50 px-3 py-1 rounded-full">
